...
export default test(({ describe })=>{
    ...
    describe("single-character test cases", ({ it })=>{
        ...
        it("renders multiple results", ()=>{
            ...
            assert.equal(renderCharacterTest(results), renderCharacterTest(createPass()) + renderCharacterTest(createFail()) + renderCharacterTest(createSkip()));
        });
    ...
    describe("single-line test cases", ({ it })=>{
        ...
        it("renders multiple results with a line feed between each one", ()=>{
            ...
            assert.equal(renderSingleLineTest(results), renderSingleLineTest(createPass()) + "\n" + renderSingleLineTest(createFail()) + "\n" + renderSingleLineTest(createSkip()));
        });
        it("renders no results as an empty string", ()=>{
    ...
    describe("multi-line test cases", ({ it })=>{
        ...
        it("renders multiple results with a two-line gap between each result", ()=>{
            ...
            assert.equal(renderMultiLineTest(results), renderMultiLineTest(createPass()) + "\n\n\n" + renderMultiLineTest(fail) + "\n\n\n" + renderMultiLineTest(createSkip()));
        });
        it("renders no results as an empty string", ()=>{
        ...
        it("renders multiple results on consecutive lines", ()=>{
            const results = [
                ...
                createSuite({
                    ...
                    children: [
                        createSkip({
                            name: "does not look inside suites",
                            mark: TestMark.skip
                        })
                    ]
                }),
            ...
            assert.equal(renderSingleLineMark(results), renderSingleLineMark(createPass({
                name: "skip 1",
                mark: TestMark.skip
            })) + "\n" + renderSingleLineMark(createPass({
                name: "none 2",
                mark: TestMark.none
            })) + "\n" + renderSingleLineMark(createPass({
                name: "only 3",
                mark: TestMark.only
            })) + "\n" + renderSingleLineMark(createSkip({
                name: "suite only 1",
                mark: TestMark.only
            })) + "\n" + renderSingleLineMark(createSuite({
                name: "suite skip 2",
                mark: TestMark.skip
            })));
        });
    ...
    describe("multi-line statuses", ({ it, describe })=>{
        ...
        it("renders timeout", ()=>{
            assert.equal(render(createTimeout({
                timeout: 500
            })), Colors.purple("Timed out after 500ms"));
        });
        describe("fail", ()=>{
            ...
            it("renders diff for assertion errors", ()=>{
                ...
                assert.equal(render(result), Colors.brightRed(error.toString()) + "\n\n" + renderDiff(error));
            });
    ...
    describe("single-word marks", ({ it })=>{
        it("renders all marks", ()=>{
            ...
            assert.equal(render(createPass({
                mark: TestMark.skip
            })), Colors.brightCyan(".skip"));
            assert.equal(render(createPass({
                mark: TestMark.only
            })), Colors.brightCyan(".only"));
        });
        function render(result) {
    ...
    describe("stack traces", ({ it })=>{
        ...
        function createResult({ stack, filename }) {
            return createFail({
                error: {
                    stack
                },
    ...
    describe("actual / expected diffs", ({ it })=>{
        ...
        it("renders expected and actual values", ()=>{
            assert.equal(render(123, "abc"), Colors.green("expected: ") + "123\n" + Colors.brightRed("actual:   ") + "'abc'");
        });
...
function renderCharacterTest(result) {
    return TestRenderer.create().renderAsCharacters(result);
}
function renderSingleLineTest(result) {
    return TestRenderer.create().renderAsSingleLines(result);
}
function renderMultiLineTest(result) {
    return TestRenderer.create().renderAsMultipleLines(result);
}
function renderSingleLineMark(result) {
    return TestRenderer.create().renderMarksAsLines(result);
}
function createSuite({ name = [], children = [], filename = undefined, mark = TestMark.none } = {}) {
    return TestResult.suite(name, children, filename, mark);
}
function createPass({ name = [], filename = undefined, mark = TestMark.none } = {}) {
    return TestResult.pass(name, filename, mark);
}
function createFail({ name = [], error = new Error("irrelevant error"), filename = undefined, mark = TestMark.none } = {}) {
    return TestResult.fail(name, error, filename, mark);
}
function createSkip({ name = [], filename = undefined, mark = TestMark.none } = {}) {
    return TestResult.skip(name, filename, mark);
}
function createTimeout({ name = [], timeout = 42, filename = undefined, mark = TestMark.none } = {}) {
    return TestResult.timeout(name, timeout, filename, mark);
}

//# sourceMappingURL=/Users/jshore/Documents/Projects/ergotest/generated/src/tests/_test_renderer_test.js.map
