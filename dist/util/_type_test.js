...
import { test, assert } from "../node_modules/tests/index.js";
...
export default test(({ describe, it })=>{
    describe("checker", ({ describe, it })=>{
        ...
        it("checks 'structs'", function() {
            ...
            assert.isNotNull(check({
                ...
                a: String
            }), "one non-matching parameter");
            assert.isNull(check({}, {}), "no parameters");
            assert.isNotNull(check({
                a: 1
            }, {}), "extra parameters should cause match to fail");
            assert.isNull(check({
                a: 1
            }, {}, {
                allowExtraKeys: true
            }), "extra parameters should NOT cause match to fail when parameter is set");
            assert.isNull(check({
                a: 1,
                b: "a"
            }, {
            ...
            assert.isNotNull(check({
                ...
                a: Number,
                b: Number
            }), "multiple with non-matching parameter");
        });
        it("supports multiple allowed types", function() {
            assert.isNull(check(1, [
                String,
                Number
            ]), "string or number");
            assert.isNotNull(check(1, [
                String,
                Object,
                Boolean
            ]), "not string, object, or boolean");
        });
        ...
        it("provides detailed error message when using structs", function() {
            assert.equal(check(null, {
                a: String
            }), "argument must be an object containing { a: <string> }, but it was null", "not a struct");
            assert.equal(check(null, {
                a: String
            }, {
                allowExtraKeys: true
            }), "argument must be an object containing at least { a: <string> }, but it was null", "not a struct, and extra keys are allowed");
            assert.equal(check({}, {
                a: String
            }), "argument.a must be a string, but it was undefined", "top-level struct");
            assert.equal(check({
                a: {
                    b: 123
                }
            ...
            }), "argument.a.b must be a string, but it was a number", "nested struct");
            assert.equal(check({
                a: "expected",
                b: "extra"
            }, {
                a: String
            }), "argument had unexpected parameter: b", "one extra parameters");
            assert.equal(check({
                a: "expected",
                b: "extra",
                c: "extra",
                ...
                a: String
            }), "argument had unexpected parameters: b, c, d", "multiple extra paraameters");
        });
        it("provides correct error message when receiving a struct and expecting a primitive", function() {
            assert.equal(check({
                id: "something"
            }, Number), "argument must be a number, but it was an object");
        });
    ...
    describe("describer", ({ describe, it })=>{
        ...
        it("describes 'structs'", function() {
            ...
            assert.equal(myDescribe({
                a: Number
            }), "object containing { a: <number> }", "one parameter");
            assert.equal(myDescribe({
                a: Number,
                b: String
            }), "object containing { a: <number>, b: <string> }");
        });
        it("describes 'structs' when extra keys are allowed", function() {
            ...
            assert.equal(myDescribe({
                a: Number
            }, options), "object containing at least { a: <number> }", "one parameter");
            assert.equal(myDescribe({
                a: Number,
                b: String
            }, options), "object containing at least { a: <number>, b: <string> }");
        });
        it("describes multiple types", function() {
            ...
            assert.equal(myDescribe([
                Boolean,
                Object
            ]), "boolean or object", "two types");
            assert.equal(myDescribe([
                Boolean,
                Object,
                Number,
                Function
            ]), "boolean, object, number, or function", "four types");
            assert.equal(myDescribe([
                undefined,
                Boolean
            ]), "undefined or boolean", "optional types shouldn't be treated specially");
            assert.equal(myDescribe([
                null,
                Object
            ]), "null or object", "nullable objects shouldn't be treated specially");
        });
        it("optionally uses articles", function() {
            ...
            assert.equal(myDescribe({
                a: Number
            }, options), "an object containing { a: <number> }");
        });
    });
});

//# sourceMappingURL=/Users/jshore/Documents/Projects/ergotest/generated/src/util/_type_test.js.map
