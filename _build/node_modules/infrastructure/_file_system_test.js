...
module.exports = suite(({ beforeAll, afterEach, describe }) => {
	...
	describe("Directory comparisons", ({ it }) => {
		...
		it("says which files have been added", async () => {
			...
			await assert.deepEqual(await fileSystem.compareDirectoriesAsync("/src", "/dest"), {
				added: [
		...
		it("does it all together, and across multiple levels", async () => {
			const fileSystem = FileSystem.createNull({
				"/src/added_file": {},
				"/src/modified_file": { modified: 1500 },
				"/subdir/dest/removed_file": {},
				"/subdir/dest/modified_file": { modified: 500 },
			});

			await assert.deepEqual(await fileSystem.compareDirectoriesAsync("/src", "/subdir/dest"), {
				added: [
					{ source: "/src/added_file", target: "/subdir/dest/added_file" }
				],
				removed: [
					{ source: "/src/removed_file", target: "/subdir/dest/removed_file" }
				],
				changed: [
					{ source: "/src/modified_file", target: "/subdir/dest/modified_file" }
				],
			});
		});

		it("filters files based on globs", async () => {
			const fileSystem = FileSystem.createNull({
				"/src/file1.a": { modified: 1500 },
				"/src/file2.a": { modified: 1500 },
				"/src/file3.b": { modified: 1500 },
				"/src/file4.ab": { modified: 1500 },
				"/dest/file1.a": { modified: 500 },
				"/dest/file2.a": { modified: 500 },
				"/dest/file3.b": { modified: 500 },
				"/dest/file4.ab": { modified: 500 },
			});

			await assert.deepEqual(await fileSystem.compareDirectoriesAsync("/src", "/dest", "**/*.*a*", "**/*.*b*" ), {
				added: [
					{ source: "/src/file1.a", target: "/dest/file1.a" },
					{ source: "/src/file2.a", target: "/dest/file2.a" },
				],
				removed: [
					{ source: "/src/file3.b", target: "/dest/file3.b" }
				],
				changed: [
					{ source: "/src/file4.ab", target: "/dest/file4.ab" }
				],
			});
		});
		...
		it("works even if directories don't exist", async () => {
			...
			await assert.deepEqual(await fileSystem.compareDirectoriesAsync("/not_exists", "/exists"), {
				added: [],
				removed: [
					{ source: "/not_exists/my_file", target: "/exists/my_file" }
				],
				changed: [],
			});
