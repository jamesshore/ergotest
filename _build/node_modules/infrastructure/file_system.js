...
module.exports = class FileSystem {
	...
	/**
	 * Same as {@link FileSystem.readFileTreeAsync}, except that it returns an empty tree rather than throwing an exception if the directory doesn't exist.
	 * @param {string} directoryName The directory to look within.
	 * @param {string | string[]} [globsToExclude] Don't include files that match these file globs.
	 * @returns {Promise<FileTree>} An object that allows further inspection of the files.
	 */
	 ...
	 * target directory.)
	 * @param {string} source The directory where the files came from
	 * @param {string} target The directory where the files are going
	 * @param {string | string[]} [sourceGlobs="**"] The files from the source to consider
	 * @param {string | string[]} [destGlobs="**"] The files from the destination to consider
	 * @returns {Promise<number>} Negative if `leftFilename` is older; positive if `rightFilename` is older; and zero if
	 *   their modification times are the same.
	 */
	async compareDirectoriesAsync(source, target, sourceGlobs = "**", destGlobs = "**") {
		ensure.signature(arguments, [ String, String, [ undefined, String, Array ], [ undefined, String, Array ]]);

		const [ sourceFiles, targetFiles ] = await Promise.all([
			this.readPossibleFileTreeAsync(source),
			this.readPossibleFileTreeAsync(target),
		]);
		ensureValidPath(source, this._rootDir);
		ensureValidPath(target, this._rootDir);

		const sourceSet = new Set(sourceFiles.matchingFiles(sourceGlobs).map(file => path.relative(source, file)));
		const targetSet = new Set(targetFiles.matchingFiles(destGlobs).map(file => path.relative(target, file)));

		const addedSet = differencePolyfill(sourceSet, targetSet);
		const removedSet = differencePolyfill(targetSet, sourceSet);
		const sameSet = unionPolyfill(sourceSet, targetSet);

		const changed = (await Promise.all([ ...sameSet ].map(async (file) => {
			const sourceFile = path.resolve(source, file);
			const targetFile = path.resolve(target, file);
			const modified = await this.compareFileModificationTimesAsync(sourceFile, targetFile) > 0;
			return modified ? sourceFile : null;
		}))).filter(file => file !== null);

		return {
			added: [ ...addedSet].map(file => path.resolve(source, file)),
			removed: [ ...removedSet].map(file => path.resolve(target, file)),
			changed,
		};
		...
		function unionPolyfill(leftSet, rightSet) {
			// Included in Node v22.0; remove this polyfill and replace with left.union(right) when that enters LTS
			return new Set([ ...leftSet ].filter(value => rightSet.has(value)));
		}
	}
