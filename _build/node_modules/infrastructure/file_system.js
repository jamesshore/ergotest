...
module.exports = class FileSystem {
	...
	async compareDirectoriesAsync(source, target, sourceGlobs = "**", destGlobs = "**") {
		...
		const [ sourceFiles, targetFiles ] = await Promise.all([
			(await this.readPossibleFileTreeAsync(source)).matchingFiles(sourceGlobs),
			(await this.readPossibleFileTreeAsync(target)).matchingFiles(destGlobs),
		]);

		const targetSet = new Set(targetFiles.map(file => path.relative(target, file)));

		const added = [];
		const changed = [];
		await Promise.all(sourceFiles.map(async (sourceFile) => {
			const relativeSource = path.relative(source, sourceFile);
			const targetFile = path.resolve(target, relativeSource);

			if (!targetSet.has(relativeSource)) {
				added.push({
					source: sourceFile,
					target: targetFile,
				});
			}
			else if (await this.compareFileModificationTimesAsync(sourceFile, targetFile) > 0) {
				changed.push({
					source: sourceFile,
					target: targetFile,
				});
			...
			targetSet.delete(relativeSource);
		}));
		const removed = [ ...targetSet ].map(relativeTarget => {
			return {
				source: path.resolve(source, relativeTarget),
				target: path.resolve(target, relativeTarget),
			};
		});
