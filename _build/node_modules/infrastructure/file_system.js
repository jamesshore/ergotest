...
module.exports = class FileSystem {
	...
	/**
	 * Given a source directory and a target directory, determine which files have been added, removed, or changed. (A file has been changed if the modification time of the source file is newer than the modification time of the target directory.)
	 * @param {string} source The directory where the files came from
	 * @param {string} target The directory where the files are going
	 * @returns {Promise<number>} Negative if `leftFilename` is older; positive if `rightFilename` is older; and zero if
	 *   their modification times are the same.
	 */
	async compareDirectoriesAsync(source, target) {
		const [ sourceFiles, targetFiles ] = await Promise.all([
			this.readFileTreeAsync(source),
			this.readFileTreeAsync(target),
		]);
		ensureValidPath(source, this._rootDir);
		ensureValidPath(target, this._rootDir);

		const sourceSet = new Set(sourceFiles.matchingFiles("**").map(file => path.relative(source, file)));
		const targetSet = new Set(targetFiles.matchingFiles("**").map(file => path.relative(target, file)));
