...
export default test(() => {
	...
	describe("nullable", () => {
		...
		it("allows output and code to be configured according to command", async () => {
			const shell = Shell.createNull({
				"my_command option1 option2": { stdout: "stdout1", stderr: "stderr1", code: 1 },
				"my_command 'option with space'": { stdout: "stdout2", stderr: "stderr2", code: 2 },
			});

			let { stdout, stderr, code } = await shell.execAsync("my_command", "option1", "option2");
			assert.equal(stdout, "stdout1", "stdout");
			assert.equal(stderr, "stderr1", "stderr");
			assert.equal(code, 1, "code");

			({ stdout, stderr, code } = await shell.execAsync("my_command", "option with space"));
			assert.equal(stdout, "stdout2", "stdout");
			assert.equal(stderr, "stderr2", "stderr");
